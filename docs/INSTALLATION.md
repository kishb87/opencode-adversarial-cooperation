# OpenCode Adversarial Cooperation Plugin - Global Setup (Auto-Load Method)

The opencode-adversarial-cooperation plugin is configured to load **globally** across all projects using OpenCode's auto-load directory.
...
ln -sf /path/to/opencode-adversarial-cooperation/plugin.ts ~/.config/opencode/plugin/opencode-adversarial-cooperation.ts

# Symlink 2: Commands directory
ln -sf /path/to/opencode-adversarial-cooperation/commands ~/.config/opencode/command/tdd
```
...
```bash
# Check symlinks
ls -la ~/.config/opencode/plugin/opencode-adversarial-cooperation.ts
ls -la ~/.config/opencode/command/tdd/

# Should see:
# opencode-adversarial-cooperation.ts -> /path/to/opencode-adversarial-cooperation/plugin.ts
# tdd -> /path/to/opencode-adversarial-cooperation/commands
```
...
**Plugin Entry Point:**
`~/.config/opencode/plugin/opencode-adversarial-cooperation.ts` → symlink to `/path/to/opencode-adversarial-cooperation/plugin.ts`
...
Scans ~/.config/opencode/plugin/ for .ts files
    ↓
Finds opencode-adversarial-cooperation.ts (symlinked to plugin.ts) ✅
    ↓
...
```bash
cd /path/to/opencode-adversarial-cooperation
...
├── plugin/
│   └── opencode-adversarial-cooperation.ts          # Entry point (symlink → plugin.ts) ✅
└── command/
...
/path/to/opencode-adversarial-cooperation/            # Your dev directory
...
Place in:
- `{project}/opencode-adversarial-cooperation.json` (highest priority)
- `{project}/.opencode/opencode-adversarial-cooperation.json`
- `~/.config/opencode/opencode-adversarial-cooperation.json` (global defaults)
...
```bash
rm ~/.config/opencode/plugin/opencode-adversarial-cooperation.ts
# Plugin no longer loads (commands still work)
```

### Option 2: Temporarily rename it
```bash
mv ~/.config/opencode/plugin/opencode-adversarial-cooperation.ts ~/.config/opencode/plugin/opencode-adversarial-cooperation.ts.disabled
...
```bash
ls -la ~/.config/opencode/plugin/opencode-adversarial-cooperation.ts
# Should show: opencode-adversarial-cooperation.ts -> /path/to/opencode-adversarial-cooperation/plugin.ts
```
...
cat ~/.config/opencode/plugin/opencode-adversarial-cooperation.ts
# Should show: export { default } from './dist/index.mjs'

# 3. Built files exist
ls -la /path/to/opencode-adversarial-cooperation/dist/
...
```bash
npm install -g opencode-adversarial-cooperation
# Or add to project: npm install opencode-adversarial-cooperation
```
